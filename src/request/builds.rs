...
impl<'a> BuildsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<BuildsResponse> {
        let mut r = self
            .http_client
            .client
            .get(
...
impl<'a> ::std::future::IntoFuture for BuildsRequest<'a> {
    type Output = httpclient::InMemoryResult<BuildsResponse>;
