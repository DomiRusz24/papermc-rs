...
impl<'a> VersionRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<VersionResponse> {
        let mut r = self
            .http_client
            .client
            .get(
...
impl<'a> ::std::future::IntoFuture for VersionRequest<'a> {
    type Output = httpclient::InMemoryResult<VersionResponse>;
